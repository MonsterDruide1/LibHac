...
internal class GameCardStorageDevice : GameCardStorageInterfaceAdapter, IStorageDevice
{
    ...
    public Result AcquireReadLock(ref SharedLock<ReaderWriterLock> outLock)
    {
        if (_isSecure)
        {
            Result rc = _manager.Get.AcquireSecureLock(ref outLock, ref _handle, _cardDeviceId, _cardImageHash);
            if (rc.IsFailure()) return rc.Miss();
        }
        else
        {
            Result rc = _manager.Get.AcquireReadLock(ref outLock, _handle);
            if (rc.IsFailure()) return rc.Miss();
        }
