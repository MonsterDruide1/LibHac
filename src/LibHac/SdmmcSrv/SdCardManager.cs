...
public class SdCardManager : IStorageDeviceManager, IStorageDeviceOperator, ISdmmcDeviceManager
{
    ...
    public Result OpenDetectionEvent(ref SharedRef<IEventNotifier> outDetectionEvent)
    {
        ...
        Result rc = _detectionEventManager.Value.CreateDetectionEvent(ref outDetectionEvent);
    ...
    public Result OpenDevice(ref SharedRef<IStorageDevice> outStorageDevice, ulong attribute)
    {
        ...
        Result rc = OpenDeviceImpl(ref storageDevice.Ref());
        if (rc.IsFailure()) return rc.Miss();
    ...
    private Result OpenDeviceImpl(ref SharedRef<IStorageDevice> outStorageDevice)
    {
        ...
        Result rc = EnsureActivated(out SdmmcHandle handle);
        if (rc.IsFailure()) return rc.Miss();
    ...
    private Result EnsureActivated(out SdmmcHandle outHandle)
    {
        ...
        if (_openCount == 0)
        {
            Result rc = SdmmcResultConverter.GetFsResult(_port, _sdmmc.Activate(_port));
            if (rc.IsFailure()) return rc.Miss();
        ...
        OutBuffer buffer2, int operationId)
    {
        ...
        switch (operation)
        {
            case SdCardManagerOperationIdValue.GetAndClearErrorInfo:
            {
                ...
                Result rc = GetAndClearSdCardErrorInfo(out buffer1.As<StorageErrorInfo>(), out bytesWrittenBuffer2,
                    buffer2.Buffer);
                if (rc.IsFailure()) return rc.Miss();
        ...
        Span<byte> logBuffer)
    {
        ...
        Result rc = Common.GetAndClearSdmmcStorageErrorInfo(out outStorageErrorInfo, out outLogSize, logBuffer, _sdmmc);
        if (rc.IsFailure()) return rc.Miss();
